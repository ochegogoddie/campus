      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {profile._count && (
            <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-200">
              <div className="text-center">
                <div className="text-2xl font-bold text-slate-900">{profile._count.postedGigs}</div>
                <div className="text-xs text-slate-600 mt-1">Gigs Posted</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-slate-900">{profile._count.createdProjects}</div>
                <div className="text-xs text-slate-600 mt-1">Projects</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-slate-900">{profile._count.applications}</div>
          ...
          <div className="space-y-4">
            {profile.bio && (
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Bio</h2>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.skills.map((skill) => (
                    <span key={skill} className="bg-amber-100 text-amber-900 px-3 py-1 rounded-full text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            {!profile.bio && !profile.skills?.length && !
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-8 text-center">
                <p className="text-slate-600 mb-4">Your profile is empty. Add a bio and skills to stand out!</p>
            ...
            {(profile.githubUrl || profile.websiteUrl || profile.linkedinUrl || profile.twitterUrl) && (
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                <h2 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Links</h2>
                <div className="flex flex-wrap gap-4">
                  {profile.githubUrl && (
                    <a href={profile.githubUrl} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-2 text-slate-700 hover:text-slate-900 text-sm transition-colors">
        ...
        {isEditing && (
          <form onSubmit={handleSave} className="bg-white border border-slate-200 rounded-xl p-6 space-y-5">
            ...
            {saveSuccess && (
              <div className="bg-emerald-50 border border-emerald-300 text-emerald-800 px-4 py-3 rounded-md text-sm">
            ...
            <div>
              ...
              <textarea
                ...
                value={form.bio}
                onChange={handleChange}
                className="w-full px-4 py-2 bg-slate-50 border border-slate-300 rounded-md text-slate-900 placeholder-slate-500 focus:outline-none focus:border-amber-500 resize-none"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Major / Field</label>
                <input
                  name="major"
                  value={form.major}
                  onChange={handleChange}
              ...
              {form.skills && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {form.skills.split(",").map((s) => s.trim()).filter(Boolean).map((skill) => (
                    <span key={skill} className="bg-amber-100 text-amber-900 text-xs px-2 py-1 rounded-full">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            {/* Social Links */}
            <div>
              <h3 className="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-3">Social Links</h3>
